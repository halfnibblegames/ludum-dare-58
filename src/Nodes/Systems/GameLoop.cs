...
public sealed partial class GameLoop : Node {
  ...
  public override void _Process(double delta) {
    if (GameIsOver) {
      ...
      if (!gameOverOverlay.Visible) {
        ...
        scoreLabel.Text =
          $"It crashed after {gameDuration.TotalMinutes:0}:{(gameDuration.TotalSeconds % 60):00} and your score is {scoreTracker?.Score ?? 0}.";
  ...
  public void InterruptGarbageCollecting() {
    ...
    Global.Services.Get<ShakeCamera2D>().Shake(0.5f);
